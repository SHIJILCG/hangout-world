export interface PlayerPreference { blocked: boolean }
    ...
    try {
      ...
      for (const entry of parsed.slice(-200)) {
        if (!Array.isArray(entry) || entry.length !== 2 || typeof entry[0] !== 'string'
          ...
          || typeof entry[1].blocked !== 'boolean') {
          throw new Error('Invalid saved preference');
        }
        if (entry[1].blocked) this.entries.set(entry[0], { blocked: true });
      }
    } catch {
      this.entries.clear();
